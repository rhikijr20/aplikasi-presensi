...
<html lang="en">
  ...
  <body class="bg-[#192f46] flex items-center justify-center min-h-screen">
    ...
    <div class="w-full max-w-xs p-6 bg-white rounded-lg shadow-md mt-32">
      <h2 class="text-center text-xl font-semibold text-black mb-6">Masuk Presensi SMKN 3 Cimahi</h2>
      @php
        $messagewarning = Session::get('warning');
      @endphp
      @if (Session::get('warning'))
        <div class="alert alert-outline-warning" style="color:red ">
          {{ $messagewarning }}
        </div>
      @endif
    ...
    <script>
      ...
      document.getElementById('loginForm').addEventListener('submit', async function(event) {
        ...
        const nisn = document.getElementById('username').value;
